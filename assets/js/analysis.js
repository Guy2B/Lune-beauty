...
export function buildPrompt(client,m){ return [ 'CLIENT', `Name: ${client.name}`, `Email: ${client.email}`, '', 'RESULT', `Score: ${m.score}`, `Texture: ${m.texture.toFixed(2)}`, `Redness: ${m.redIdx.toFixed(2)}`, `Oil Balance: ${m.oilBal.toFixed(2)}`, `Pores: ${m.poreProb.toFixed(2)}`, `Pigmentation: ${m.pigment.toFixed(2)}` ].join('\n'); }
